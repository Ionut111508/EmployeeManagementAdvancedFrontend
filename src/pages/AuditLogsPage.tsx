...
export function AuditLogsPage() {
  ...
  return <section className="page-stack">
    <PageHeader eyebrow="Governance" title="Audit log" description={session?.role === 'Manager' ? 'Actions performed by employees assigned to your managed projects.' : 'Traceable history of task, allocation and timesheet changes.'} />
    ...
    {logs.data && visible.length > 0 && <div className="table-card"><table className="data-table"><thead><tr><th>Time</th><th>Actor</th><th>Action</th><th>Entity</th><th>Project</th><th>Summary</th></tr></thead><tbody>{visible.map(item => <tr key={item.auditLogId}><td>{formatDate(item.createdAt)}<br/><span className="muted">{new Date(item.createdAt).toLocaleTimeString('ro-RO')}</span></td><td>{item.actorName ?? item.actorEmployeeId ?? 'System'}{item.actorEmployeeId && <><br/><span className="muted">{item.actorEmployeeId}</span></>}<br/><span className="badge">{item.actorRole}</span></td><td><span className="badge">{item.action}</span></td><td>{item.entityType}<br/><span className="muted">{item.entityId}</span></td><td>{item.projectId ?? '-'}</td><td>{item.summary}</td></tr>)}</tbody></table></div>}
  </section>;
}
