...
export function TasksPage() {
  ...
  return <section className="page-stack">
    <PageHeader eyebrow="Task management" title="Tasks" description="Tasks visible for your current role." actions={hasPermission('tasks.manage') || hasPermission('tasks.manage.managed') ? <Link className="btn" to="/tasks/create">Create task</Link> : undefined} />
    ...
    {hasPermission('allocations.simulate') && <div className="table-card">
      ...
      {understaffed.length > 0 && <table className="data-table"><thead><tr><th>Task</th><th>Planned period</th><th>Required skill</th><th>People</th><th>Estimated</th><th>Allocated</th><th>Missing</th><th>Action</th></tr></thead><tbody>{understaffed.map(item => <tr key={`${item.projectId}-${item.taskId}`}><td><strong>{item.taskName}</strong><br/><span className="muted">{item.projectName}</span></td><td>{formatDate(item.plannedStartDate)} - {formatDate(item.plannedEndDate)}</td><td>{item.requiredSkillName ? `${item.requiredSkillName} ${item.requiredSkillLevel ?? ''}` : '-'}</td><td>{item.allocatedPeople}</td><td>{formatNumber(item.estimatedHours)}h</td><td>{formatNumber(item.allocatedHours)}h</td><td><span className="badge">{formatNumber(item.remainingHours)}h</span></td><td><Link className="btn" to={`/allocations/create?projectId=${encodeURIComponent(item.projectId)}&taskId=${encodeURIComponent(item.taskId)}`}>Find employee</Link></td></tr>)}</tbody></table>}
    </div>}
    ...
    {data && session?.role !== 'Employee' && <TaskTable title="Visible tasks" tasks={filteredTasks} emptyText="No tasks match the selected filters." />}
  </section>;
}
