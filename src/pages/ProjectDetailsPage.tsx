...
export function ProjectDetailsPage() {
  ...
  return <section className="page-stack">
    ...
    {data && <>
      ...
      <div className="table-card"><h2>Tasks</h2><table className="data-table"><thead><tr><th>Task</th><th>Status</th><th>Description</th><th>Estimated</th><th>Action</th></tr></thead><tbody>{data.projectTasks.map(task => <tr key={task.taskId}><td><strong>{task.taskName}</strong><br/><span className="muted">{task.taskId}</span></td><td><span className={`badge task-${task.status.toLowerCase()}`}>{task.status}</span></td><td>{task.description?.taskDescriptionText ?? task.descriptionId}</td><td><span className="badge">{formatNumber(task.estimatedHours)}h</span></td><td><Link className="btn-link" to={`/task-view/${encodeURIComponent(task.projectId)}/${encodeURIComponent(task.taskId)}`}>View task</Link></td></tr>)}</tbody></table></div>
