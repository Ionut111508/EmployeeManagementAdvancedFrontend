...
export function GanttPage() {
  ...
  return <section className="page-stack">
    ...
    <div className="card filter-bar"><strong>Filters</strong><select className="field" value={projectFilter} onChange={e => setProjectFilter(e.target.value)}><option value="all">All projects</option>{projects.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select>{session?.role !== 'Employee' && <select className="field" value={employeeFilter} onChange={e => setEmployeeFilter(e.target.value)}><option value="all">All employees</option>{employees.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select>}<select className="field" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="all">All statuses</option><option value="active">Active now</option><option value="future">Future</option><option value="closed">Completed</option></select><span className="badge">{filtered.length} allocations</span></div>
      ...
      <div className="gantt-list">
        {filtered.map(item => {
          ...
          return <div className="gantt-row" key={`${item.employeeId}-${item.projectId}-${item.taskId}`}>
            ...
            <div className="gantt-track"><div className="gantt-bar" tabIndex={0} style={{ left: `${left}%`, width: `${width}%` }}><span className="gantt-bar-label">{formatDate(item.allocationStartDate)} - {formatDate(item.allocationEndDate)}</span><div className="gantt-tooltip" role="tooltip"><strong>{item.taskName ?? item.taskId}</strong><span>{item.employeeName ?? item.employeeId}</span><span>{item.projectName ?? item.projectId}</span><span>{formatDate(item.allocationStartDate)} - {formatDate(item.allocationEndDate)}</span><span>{formatNumber(item.allocatedHours)}h/day, {formatNumber(totalHours)}h total</span><span>{status}{item.requiredSkillName ? `, ${item.requiredSkillName} ${item.requiredSkillLevel ?? ''}` : ''}</span></div></div></div>
          </div>;
        })}
      </div>
    </div>}
